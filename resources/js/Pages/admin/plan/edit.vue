   ...
   <AuthenticatedLayout>
    <section class="min-h-[95vh] bg-indigo-100 drop-shadow-2xl p-5 rounded-2xl">
        ...
        <div class="flex m-1 p-2 justify-end">
          ...
          <Link :href="route('admin.plan.index')" class="btn-indigo">Plan Table</Link>
        </div>
  

        <!-- form -->
        <form @submit.prevent="submit"  class="w-10/12 lg:w-5/12 mx-auto bg-gray-100 p-8 my-8 rounded-xl">
            <h4 class="text-3xl font-bold m-1 text-center"> Plan Edit </h4>
            <div>
...
const submit = ()=> {
    form.put(route("admin.plan.update",props.plan.slug),{
          onSuccess: ()=> cleanForm()
    });
}
...
function cleanForm(){
    form.reset();
    toastr.success("Plan Data Update Successfully");
}
