...
function cleanForm(){
    ...
    form.reset();
    toastr.success("Plan Data Added Successfully");
...
<template>
  <AuthenticatedLayout>
    <section class="min-h-[95vh] bg-slate-50 drop-shadow-2xl p-5 rounded-lg">
      <!--  -->
      <h4 class="text-3xl font-bold m-1 py-4"> Plan  </h4>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-5 gap-4">
        ...
        <form @submit.prevent="submit"  class="w-full mx-auto bg-gray-100 p-4 rounded-xl">
          ...
          <div class="mt-4">
              ...
              <TextInput id="priority" v-model="form.priority" type="number"  class="mt-1 block w-full" placeholder="Enter Priority"/>
        ...
        >
          ...
          >
            <tr>
              ...
              <th scope="col" class="px-3 py-2 border border-slate-500">Priority</th>
              <th scope="col" class="px-3 py-2 border border-slate-500">Action</th>
            </tr>
          </thead>
          <tbody v-if="plans.data.length > 0">
            <tr v-for="(plan, index) in plans.data" :key="plan.index" class="dark:bg-gray-800 dark:border-gray-700">
              <td class="px-3 py-2 border border-slate-500"> {{index+1}}</td>
              <td class="px-3 py-2 border border-slate-500 capitalize"> {{ plan.name }} </td>
              <td class="px-3 py-2 border border-slate-500 capitalize"> {{ plan.slug }} </td>
              <td class="px-3 py-2 border border-slate-500"> {{ plan.priority }} </td>
              <td class="px-3 py-2 border border-slate-500 ">
                 <div class="flex justify-start items-center gap-4">
                    <div>  <Link :href="route('admin.plan.edit',plan.slug)" class="btn-edit "> <i class="fa-solid fa-pen-to-square"></i></Link></div>
                  
                  <div>  <Link :href="route('admin.plan.destroy',plan.slug)" onclick="return confirm('Are you sure you want to delete this item?');"  as="button" type="button" method="delete" class="btn-delete"> <i class="fa-solid fa-trash"></i> </Link></div>
                 </div>
              </td>
            </tr>
          </tbody>
